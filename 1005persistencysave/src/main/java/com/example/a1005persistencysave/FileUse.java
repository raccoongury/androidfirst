...
//                    edit.setText(new String(b));
            ...
            case R.id.imagecopy:
                try(
                        InputStream fis = getResources().openRawResource(R.raw.lion);

                        FileOutputStream fos = openFileOutput("lion.png", Context.MODE_PRIVATE);
                ) {
                    while (true){ byte []b = new byte[1024];
                    //fis에서 읽어서 b에 저장
                    int r = fis.read(b);
                    //읽은게 없으면 종료
                    if (r <= 0) break;
